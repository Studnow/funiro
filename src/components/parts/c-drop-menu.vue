  ...
  <div class="w-full max-w-sm px-2">
    <Popover v-slot="{ open }" class="relative">
      <PopoverButton
        ...
        class="pop-button trans-150-eio group"
      >
        <span>{{ menuItemName }}</span>
        <ChevronDownIcon
          :class="open ? '' : 'text-opacity-70'"
          class="down-icon trans-150-eio"
          aria-hidden="true"
        />
      </PopoverButton>
...
.down-icon {
  @apply w-5 h-5 ml-2 text-black group-hover:text-primary group-focus:text-primary;
}
.pop-panel {
  @apply absolute z-20 w-max max-w-sm px-4 mt-3 transform -translate-x-1/2 left-1/2 sm:px-0;
}
</style>